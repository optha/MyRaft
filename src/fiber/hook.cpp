...
namespace monsoon
{
    ...
    static int g_tcp_connect_timeout = 500;
...
#undef XX
        ...
        unsigned int sleep(unsigned int seconds) {
            ...
            // 允许hook,则直接让当前协程退出，seconds秒后再重启
            Fiber::ptr fiber = Fiber::GetThis();
            IOManager *iom = IOManager::GetThis();
            iom->addTimer(seconds * 1000, std::bind((void(Scheduler::*)(Fiber::ptr, int thread)) & IOManager::scheduler, iom, fiber, -1));
            Fiber::GetThis()->yield();
            return 0;
        }
